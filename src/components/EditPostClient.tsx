...
export default function EditPostClient({
  ...
  useEffect(() => {
    ...
    if (initialPostData === undefined && slug) {
      const fetchPost = async () => {
        ...
        try {
          console.log(
            `Fetching post with slug: ${slug} in client component (fallback)`,
          );
      ...
      if (initialPostData && (!user || initialPostData.authorId !== user.uid)) {
        ...
        const checkAuth = () => {
          if (!user || initialPostData.authorId !== user.uid) {
            console.warn(
              "User is not the author based on initial data. Redirecting.",
            );
            router.push("/");
          }
        };
        ...
        if (!loadingAuth) {
          ...
          const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
            if (!currentUser || initialPostData.authorId !== currentUser.uid) {
              console.warn(
                "User is not the author after auth load. Redirecting.",
              );
              router.push("/");
            }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      !user ||
      initialPostData?.authorId !== user.uid ||
      !postId ||
      isSubmitting ||
      isUploading
    ) {
      console.warn(
        "Submission blocked: Not authorized, no post ID, submitting, or uploading.",
      );
      return;
    }
    ...
    if (imageFile) {
      ...
      try {
        ...
        setUploadError(
          "Falha ao enviar a nova imagem. Por favor, tente novamente.",
        );
      ...
      try {
        ...
        await deleteObject(oldImageRef);
        console.log(
          "Old image deleted successfully because imageUrl was cleared",
        );
        finalImageUrl = ""; // Clear the finalImageUrl as well
      } catch (deleteError) {
        console.warn(
          "Could not delete old image after clearing URL:",
          deleteError,
        );
      }
    ...
    try {
      ...
      let errorMessage =
        "Falha ao atualizar a postagem. Por favor, tente novamente.";
      if (error.code === "permission-denied") {
        errorMessage =
          "Erro de Permissão: Você não tem permissão para atualizar esta postagem. Verifique suas regras de segurança do Firestore.";
  ...
  if (
    loadingAuth ||
    loadingPost ||
    (initialPostData === undefined && (!user || !slug))
  ) {
    return (
      <div className="text-center mt-8">Carregando postagem para edição...</div>
    );
  }
  ...
  if (!postId) {
    // If no initial data AND no post found after fallback fetch
    return (
        ...
        <div>
          ...
          {(imageUrl || (initialPostData?.imageUrl && !imageFile)) && ( // Show if there's an imageUrl state or initial imageUrl and no new file
            <div className="mt-2">
              ...
              <img
                src={imageUrl || initialPostData?.imageUrl}
          ...
          {!imageUrl &&
            !imageFile &&
            !initialPostData?.imageUrl && ( // Show message if no image at all
