  ...
  return (
    ...
    >
      <CarouselContent className="-ml-4">
        {posts.map((post) => (
          <CarouselItem key={post.id} className="pl-4 sm:basis-1/2 lg:basis-1/3">
            <div className="p-1 h-full">
              <Card className="flex flex-col h-full shadow-xl hover:shadow-2xl transition-shadow duration-300 bg-card overflow-hidden group">
                <CardHeader className="p-0">
                  <Link href={`/postagens/${post.slug}`} className="block relative w-full h-48">
                    <Image
                      src={post.imageUrl}
                      alt={post.title}
                      layout="fill"
                ...
                <CardContent className="flex-grow p-4">
                  ...
                  <div className="text-xs text-card-foreground/70 mb-2 flex flex-col sm:flex-row sm:items-center sm:space-x-3">
                    <span className="flex items-center mb-1 sm:mb-0">
                      <User size={12} className="mr-1 text-accent" /> {post.author}
                    </span>
                    <span className="flex items-center">
                      <CalendarDays size={12} className="mr-1 text-accent" /> {new Date(post.date).toLocaleDateString('pt-BR')}
                    </span>
                  </div>
                ...
                <CardFooter className="p-4 pt-0">
                  <Button asChild variant="link" className="text-accent hover:text-accent/80 p-0 text-sm">
                    <Link href={`/postagens/${post.slug}`} className="flex items-center">
