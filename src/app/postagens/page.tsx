...
import {
  ...
  DocumentSnapshot,
  Timestamp,
} from "firebase/firestore"; // Import Firestore functions, add Timestamp
...
import {
  ...
  CardHeader,
  CardTitle,
} from "@/components/ui/card"; // Example paths - ADJUST THESE
...
export default function PostsPage() {
    ...
    try {
      ...
      const postsData = querySnapshot.docs.map((doc) => {
        ...
        return {
          ...
          excerpt:
            rawData.excerpt ||
            rawData.content?.substring(0, 150) ||
            "Sem resumo", // Provide fallback or use truncated content
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl uppercase-ancient font-bold mb-6">
        Últimas Postagens
      </h1>

      {posts.length === 0 &&
        !loading &&
        lastVisible === null && ( // Show message only if no posts were ever loaded
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {posts.map((post) => (
          ...
          <div key={post.id} className="p-1 h-full">
            {" "}
            {/* Outer div with padding/height */}
      ...
      <div className="flex justify-center mt-8">
        ...
        {!hasMore && posts.length > 0 && !loading && (
          <p className="text-center text-gray-600">
            Todas as postagens foram carregadas.
          </p>
        )}
      </div>
    </div>
