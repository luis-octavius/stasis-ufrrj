...
import {
  ...
  updateMember,
  deleteMember,
} from "../../../lib/integrantes"; // Import Firestore functions for members
...
import {
  ...
  CardTitle,
  CardFooter,
} from "@/components/ui/card"; // Assuming shadcn/ui cards for member list display
...
export default function AdminIntegrantesPage() {
      ...
      setMembers((prevMembers) =>
        prevMembers.map((member) =>
          member.id === editingMemberId
            ? { ...member, ...formData, id: editingMemberId }
            : member,
        ),
      );
      resetForm(); // Limpa o formulário
  ...
  const handleDeleteMember = async (memberId: string) => {
    if (!user || !confirm("Tem certeza que deseja remover este integrante?"))
      return; // Confirmação antes de deletar
